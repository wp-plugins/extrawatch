...
class ExtraWatchSetupJoomla implements ExtraWatchSetup
{
  ...
  function activate()
  {
    ...
    $this->database->setQuery(trim($query));
    $this->database->query();
    $query = "DELETE FROM #__components where admin_menu_link like '%option=com_extrawatch%'  and admin_menu_img like '%../components/com_extrawatch/img/icons/%' and admin_menu_img<>'../components/com_extrawatch/img/icons/extrawatch-logo-16x16.png'";
    $this->database->setQuery(trim($query));
    $this->database->query();

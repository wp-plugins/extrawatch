  ...
  /*  function countryByIp($ip)
    ...
    function removeRepetitiveTitle($title)  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
    {
        if ($this->config->getCheckboxValue("EXTRAWATCH_HIDE_REPETITIVE_TITLE")) {  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
            $title = str_replace($this->env->getTitle(), "", $title);  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
        }
        return $title;  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
    }
...
function renderHTMLCodeSnippet($projectId) {   ///
    ...
    if (@_EW_CLOUD_MODE) {
		...
		if (@get_class($this->env) == "ExtraWatchWordpressEnv") {
			$title .= ", Wordpress visitor counter, Wordpress visitor tracking";
		} else {
...
static function getTimezoneOffsetByTimezoneName($userTimezoneName){  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
    ...
    /**
     * Helper function to prevent restricted %2F in URI path
     * @param $str
     * @return mixed
     */
    static function unescapeSlash($str) {
        return str_replace("_SLASH_","/", $str);
    }


    function getUserByUserId($database, $userId) {
        $result = $database->objectListQuery(sprintf("select * from global_extrawatch_user where `id` = '%d' order by id asc limit 1", (int) $userId));

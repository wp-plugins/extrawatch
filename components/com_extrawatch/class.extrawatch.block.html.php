...
class ExtraWatchBlockHTML
{
    ...
    function renderBlockedIPs($date = 0, $expanded = FALSE, $displayedInStats = TRUE)
    {
        ...
        $output = "";
        if (!$expanded) {
            $output .= "<a href='javascript:extraWatchBlockIpManually();'>" . (_EW_STATS_IP_BLOCKING_ENTER) . "</a><br/>";
        }
        ...
        if (@ $rows) {
            foreach ($rows as $row) {
                ...
                $mapsIcon = "<img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/map_icon.gif' border='0'  " . $this->extraWatch->helper->getTooltipOnEvent() . "=\"ajax_showTooltip('" . $this->extraWatch->config->getLiveSite() . $this->extraWatch->env->getEnvironmentSuffix() . "components/com_extrawatch/ajax/tooltip.php?rand=" . $this->extraWatch->config->getRand() . "&ip=" . @ $row->ip . "&env=" . $this->extraWatch->config->getEnvironment() . "',this);return FALSE\"/>";
                ...
                if (!$expanded) {
                    $output .= "<a  id='$row->ip' href='javascript:extraWatchBlockIpToggle(\"$row->ip\");' title='$row->reason'>" . _EW_BLOCKING_UNBLOCK . "</a>";
                } else {

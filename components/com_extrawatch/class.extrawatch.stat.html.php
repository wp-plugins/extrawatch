...
class ExtraWatchStatHTML
{
    ...
    function changeURI($j, $row)
    {
        ...
        $row->value = "<table><tr><td>" . $row->value . "</td><td><a href='" . $this->extraWatch->config->renderLink("goals", "action=insert&uri=$uriEncoded") . "' title='" . _EW_STATS_ADD_TO_GOALS . "'><img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/goal.gif' border='0'/></a></td></tr></table>";
    ...
    {
        if ($row->name) {
            $icon = "<img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/" . strtolower($row->name) . ".gif' />";
        }
        return @$icon;
    }
    ...
    function changeCountry($j, $row, $frontend)
    {
        if ($row->name) {
            ...
            $icon = "<img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/flags/" . strtolower($row->name) . ".png' title='$countryName' alt='$countryName'/>";
        }
    ...
    <img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/goal.gif' border='0'/></a></td></tr></table>";
    ...
    function renderVisitsGraph($week = 0)
    {
        ...
        for ($day = $startDay; $day < $startDay + 7; $day++) {
            ...
            foreach ($stats as $key => $value) {
                ...
                $progressBarIcon = $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/progress_bar_$key.gif";

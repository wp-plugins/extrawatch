...
class ExtraWatchGoalHTML
{
    ...
    {
        $output = "";
        $output .= "<a href='" . $this->extraWatch->config->renderLink("goals", "action=edit&goalId=$id") . "'><img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/edit.gif' border='0' title='" . _EW_GOALS_EDIT . "'/></a> ";
        $output .= "&nbsp;&nbsp;";
        $output .= "<a href='" . $this->extraWatch->config->renderLink("goals", "action=delete&goalId=$id") . "' onClick='return confirm(\"" . _EW_GOALS_DELETE_CONFIRM . " $id? \");'><img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/delete.gif' border='0' title='" . _EW_GOALS_DELETE . "'/></a> ";
        return $output;
    }
    ...
    {
        echo ("<h2>" . _EW_GOALS_TITLE . ExtraWatchHTML :: renderOnlineHelp(DB_KEY_GOALS) . "</h2>");
        if ($result) {
            echo (_EW_SUCCESS . "<br/>");
        }
    ...
    {
        if ($disabled)
            $output = "<a href='" . $this->extraWatch->config->renderLink("goals", "action=enable&goalId=$id") . "'><img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/unpublished.png' border='0'/></a>";
        else
            $output = "<a href='" . $this->extraWatch->config->renderLink("goals", "action=disable&goalId=$id") . "'><img src='" . $this->extraWatch->config->getLiveSiteWithSuffix() . "components/com_extrawatch/icons/published.png' border='0'/></a>";

...
$id = $extraWatch->database->resultQuery($query);

$query = sprintf("update #__updates set extension_id = %d where extension_id = 0", (int) $id);
$id = $extraWatch->database->executeQuery($query); ?>

<h2>Update ExtraWatch (for Joomla 1.6 and above)</h2>
...
1. <b><a href="<?php echo $extraWatch->env->getRootSite();?>administrator/index.php?option=com_installer&view=update">Open update manger</a></b> <br/><br/>

2. Click Find Updates<br/><br/>

3. Go back to ExtraWatch -> Update and <b><a href="<?php echo $extraWatch->env->getRootSite();?>administrator/index.php?option=com_installer&view=update">Open
